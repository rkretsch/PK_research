...
double RNA::GetEnsembleDefect(const int structurenumber, const int start, const int end) {
	//check to see if partition function data is present.
	if (!partitionfunctionallocated)
		PartitionFunction();
	...
	for(int i=pos_start;i<=pos_end;++i){//for every nucleotide in the structure
		...
		if (GetPair(i, structurenumber) == 0) {
			...
			double factor;
			if (GetPair(i, structurenumber) > pos_end) factor = 1.0;
			else factor = 2.0;

			defect += factor * (1 - GetPairProbability(i, GetPair(i, structurenumber)));
		}
